...
public class PigDiceGame {
	public static void main(String[] args) {
		...
		while (totalscore < 20) { // run a loop until the total score is less than 20
			...
			if (RollorHold == 'r') { // if it is r we roll the dice
				int diceroll = (int) Math.floor(Math.random() * (6) + 1); //  this gives a random number between 1 and 6 as a dice
				System.out.println("You rolled: " + diceroll);
				if (diceroll == 1) { // if the dice roll is 1 we end the turn and make the turn score zero as turn ended
					...
					turnscore += diceroll; // we add the dice value until the user holds the turn score
					if (turnscore > 20) { // if the turn score is greater than 20, the user can hold and finish the game 
